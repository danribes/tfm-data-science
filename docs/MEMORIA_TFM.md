...
Los resultados delimitan lo que el sistema puede sostener. El trabajo emplea dos redes neuronales y conviene no confundirlas: el codificador multilingüe de la búsqueda documental —un transformador de 24 capas que produce vectores de 1.024 dimensiones— está en producción y se ejecuta en cada consulta, y con él se midió el `hit@8` publicado; el perceptrón multicapa entrenado para proyectar el precio de la vivienda no supera el criterio de comparación con una extrapolación de tendencia y no interviene en ninguna proyección. La clasificación soberana presenta discriminación moderada y no está calibrada para España. La aparente precisión de la media regional de vivienda disminuye sustancialmente al preservar los movimientos nacionales comunes en el remuestreo: el 3 % de la calibración heredada deja de rechazarse. De ocho parámetros estructurales examinados, sólo tres resultan identificables con los datos disponibles, y los cinco restantes se declaran no identificados con su motivo econométrico.
...
Una advertencia sobre lo que se consulte ahí: las 35 preguntas participaron en el ajuste, así que el `hit@8` es una cota superior de desarrollo. Durante un tiempo hubo una segunda —el despliegue resolvía por BM25 y no por la fusión híbrida con la que se midió esa cifra— y merece la pena dejar escrito por qué, porque el fallo fue de diseño antes que de configuración. El endpoint de inferencia que el código llevaba escrito fue retirado por el proveedor y dejó de resolver en DNS, de modo que la mitad densa caía en cada consulta; el campo que anunciaba el modo de recuperación era el valor estático de la configuración y seguía diciendo «híbrido». Un campo que informa de la intención en lugar del hecho es peor que no tenerlo. Hoy cada respuesta publica el recuperador que de verdad la resolvió.
