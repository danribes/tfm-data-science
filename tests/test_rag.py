...
# ---- qué puede citarse como fuente -------------------------------------------

def test_solo_responden_las_fuentes_que_lo_son():
    """Un trabajo no puede citarse a sí mismo como autoridad.

    El corpus llevaba tres colecciones propias y una vista que las fundía con
    los manuales. El problema no era la idea sino el contenido: de los quince
    documentos propios, los mayores eran planes de desarrollo —88 fragmentos de
    uno de implementación, 41 de un extracto del motor— frente a 23 entre la
    memoria, los resultados y la reproducibilidad. Ganaban el ranking, así que
...
def test_la_opinion_sigue_separada_de_los_manuales():
    """`crack23` se queda, pero como lo que es: opinión etiquetada. La regla
    del módulo nunca fue «una colección cada vez», sino no enfrentar
    autoridades distintas en una misma lista."""
