...
describe("Evidencia — calibrado frente a estimado", () => {
  ...
  it("splits the housing window so the estimate's sample dependence shows", async () => {
    ...
    expect(within(bust).getByText("−6,45")).toBeInTheDocument();  // U+2212, per fmt.nf
    const boom = screen.getByText(/2014–2026/).closest("tr")!;
    expect(within(boom).getByText("4,97")).toBeInTheDocument();
  ...
  it("puts the estimated impulse response next to the engine's assumption", async () => {
    ui();
    await waitFor(() => expect(screen.getByText(/Crecimiento previo y cambio acumulado/)).toBeInTheDocument());
    const s = screen.getByText(/Crecimiento previo y cambio acumulado/).closest("section")!;
    const legend = s.querySelector(".legend")!;
    expect(within(legend as HTMLElement).getByText(/estimado en el panel/)).toBeInTheDocument();
    expect(within(legend as HTMLElement).getByText(/puntos anuales normalizados/)).toBeInTheDocument();
    ...
    expect(within(s).getByText(/20 % cada año/)).toBeInTheDocument();
  });

  it("does not interpret an accumulating level gap as evidence against growth reversion", async () => {
    ui();
    await waitFor(() => expect(screen.getByText(/Crecimiento previo y cambio acumulado/)).toBeInTheDocument());
    const s = screen.getByText(/Crecimiento previo y cambio acumulado/).closest("section")!;
    expect(within(s).getByText("0,56")).toBeInTheDocument();
    expect(within(s).getByText(/no valida el motor/)).toBeInTheDocument();
    expect(within(s).queryByText(/inercia, no reversión/)).not.toBeInTheDocument();
  });

  it("declares the descriptive normalization and lack of causal identification", async () => {
    ui();
    await waitFor(() => expect(screen.getByText(/Crecimiento previo y cambio acumulado/)).toBeInTheDocument());
    const s = screen.getByText(/Crecimiento previo y cambio acumulado/).closest("section")!;
    expect(within(s).getByText(/sin identificación causal/)).toBeInTheDocument();
  });
